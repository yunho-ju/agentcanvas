...
- **Skills.** A skill is a written way of working (the open `SKILL.md` format) that an AI step follows. Bring one in by pasting, by address (skills.sh or GitHub — read in full before it is added), or by searching; pick a starter; or turn a step's own instruction into a skill with a drafted structure. Skills live in the document next to its connections, show up in the run as "follows: …", and the Architect draft can pick them for a step. Scripts inside a skill are never run.
- **Input rows with a kind.** Say whether each value the run asks for is text, a number, a yes-or-no, a list or a bundle, and whether it is required; the run card asks with the matching field and connections are checked against it while you draw.
