...
chat-panel (대화 — 게시된 판과 말을 주고받는 화면, CHAT-3b)
...
- 구조(위에서 아래로): 헤더[제목 '대화' --text-title + 판 고정 표시 1줄] → 말풍선 스택(간격 var(--space-2), 내부 스크롤) → 입력줄 → 아래 손잡이 행 ['새 대화' ghost]['대화 지우기' ghost]
...
- 문구 전부 사전 경유(ko/en 쉬운 말). 세션 안의 대화만 화면에 남는다 — 새로고침 뒤 이력 복원과 지난 대화 목록은 후속(CHAT-4)이고, 이 화면이 그것을 지원한다고 말하지 않는다
```
