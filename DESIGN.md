...
- 놓이는 자리(UXQ-5): 선택된 노드가 있으면 그 노드 오른쪽(실측 폭 + `--space-6`, 같은 y). 없으면 **뷰포트 가운데 줄에 이미 선 카드 중 가장 오른쪽 카드의 오른쪽**(실측 폭 + `--space-4`). 두 경우 모두 아래 '화면 안이 먼저다'의 줄 유지 조건을 따른다. 그 줄에 카드가 없거나 줄이 화면 폭을 넘으면 아래의 격자 탐색. 선택이 있든 없든 연달아 놓은 카드는 한 줄에 읽는 순서로 선다. 자동 연결은 하지 않는다(오배선 금지)
