...
- **되묻기 상태(pattern-asks, AGENT_PATTERNS P6)**: 입력과 review 사이에 설 수 있는 **선택적** 상태다. 서버가 부탁 문장을 읽고 "이런 게 필요한가요"를 **최대 세 개**까지 돌려줄 때만 선다(없으면 지금처럼 곧장 review). 한 시점에 하나만 묻는 규칙은 그대로다 — 카드 제목이 그 질문 문장이고, 아래 캡션 한 줄이 그 대가(cost)다. 버튼 행 [예][아니요][건너뛰기 ghost]. 진행 표시는 '3개 중 2번째'(--text-caption) 한 줄뿐이다(진행 막대·단계 점 금지)
- 되묻기는 **한 번뿐이다**: 답을 모아 다시 물으면 서버는 초안을 내놓아야 하고 또 묻지 못한다(설문지가 되지 않는다). '건너뛰기'는 '아니요'와 다르다 — 모른다는 답이라 서버에 그대로 전한다. 되돌아가는 길은 Esc(§1 체인 — 카드를 닫고 적던 부탁은 남는다)와 마지막 질문의 '다시 적기'다
- 질문·대가 문구는 **서버 카탈로그의 것을 그대로 쓴다**(화면이 문장을 짓지 않는다). 패턴의 코드 이름(react·supervisor 같은 원명)은 화면·접근성 이름 어디에도 쓰지 않는다 — 사람이 읽는 것은 질문·대가 두 문장뿐이다
- 답이 '예'인 모양은 **서버가** 초안에 넣는다(모델이 구조를 지어내지 않는다). 그렇게 만들어진 초안도 기존 검사 3종과 승인 절차를 그대로 탄다 — 되묻기가 승인 없는 지름길이 되지 않는다
...
- **가짜 실행은 사람 확인 앞에서 멈추지 않는다**(2026-09-04 실측): 이 검사가 묻는 것은 '이 그래프가 끝까지 걸어지는가'라는 구조의 사실이지 '사람이 승인할까'라는 정책이 아니다. 멈춘 자리마다 승인으로 이어 걷고(`resumeFakeRun`), 그렇게 끝까지 가면 통과다. 그러지 않으면 **사람 확인 카드가 든 그래프는 무엇이든 이 검사를 통과할 수 없어** 적용이 영영 막힌다(예: '사람이 확인하고 넘어가기'에 '예'라고 답해 만든 초안, 그리고 확인 카드가 든 기존 문서의 고치기 후보). 이어 걷다가 다시 멈추는 자리가 없어질 때까지 반복하되, 한 걸음도 나아가지 못하면 실패다(끝나지 않는 되풀이를 만들지 않는다)
- 고치기 카드(§7 optimize-card)도 **같은 게이트를 쓴다** — 검사가 하나라도 실패한 후보는 적용을 막고 비활성 이유를 말한다. 두 카드가 같은 검사를 두 기준으로 읽지 않는다
...
- **이 모양으로 놓기(AGENT_PATTERNS P6)**: 팔레트 목록 아래 구분선 한 줄 뒤에 서버가 놓아 줄 수 있는 모양들이 선다(`GET /patterns`, 없으면 이 구역 자체가 없다). 한 줄 = [모양의 질문을 줄인 이름 --text-label] + [대가 한 줄 --text-caption ink-soft]. 누르면 그 모양의 카드·선이 한 번에 놓이고 **되돌리기 한 걸음**이다. 지금 고른 노드가 그 모양의 자리(에이전트 등)면 그 노드에 놓고, 고른 것이 없고 후보가 여럿이면 놓지 않고 그 자리에서 말한다 — '어느 단계에 놓을지 골라 주세요'(서버가 돌려준 못 채우는 까닭을 그대로 쓴다). 도구가 필요한 모양인데 그 단계에 도구가 없으면 같은 규칙으로 '먼저 이 단계가 쓸 도구를 골라 주세요'. **조용히 아무 일도 일어나지 않는 길을 만들지 않는다**
...
- **턴 묶음(run-turns, AGENT_PATTERNS P4)**: 한 노드가 도구를 부르며 여러 번 시도한 실행(`turn` 필드가 있는 사건들)은 같은 `turn`의 줄을 **한 묶음**으로 들여쓰고, 묶음 머리말 한 줄(--text-label)로 그 시도를 쉬운 말로 요약한다. 묶음의 정체는 **(노드, turn)**이다 — 승인으로 끊겼다 이어져도 한 묶음이고, 머리말은 **한 번만** 선다(이어진 뒷부분은 들여쓰기만 물려받는다). `turn`이 없는 사건은 지금과 같다(묶음 없음). 묶음 머리말은 줄이 아니라 제목이다 — 누를 수 없고 aria-current를 갖지 않는다
- **머리말은 일어난 일만 말한다**(추측 금지 — 실행 보기는 살아 있는 화면이다): 문구는 **관찰된 사건**에서 고른다. 아직 답이 안 온 시도는 "{n}번째 시도 — 물어보는 중이에요"(--text-caption, running 색) / 실제로 부른 도구가 있으면(그 묶음의 `tool.requested`) "{n}번째 시도 — '{도구}' 도구를 불렀어요", 둘이면 "'{A}'·'{B}' 도구를 불렀어요", 셋 이상이면 "{n}번째 시도 — 도구 {개수}개를 불렀어요" + caption에 원문 이름 전부 / 도구 없이 답했으면 "{n}번째 시도 — 답했어요" / 마무리 호출(`closing`)은 "여기까지 알아본 것으로 답했어요"이고 **도구 이름을 붙이지 않는다**(마무리에서 모델이 시킨 도구는 엔진이 부르지 않는다 — 부르지 않은 것을 적지 않는다)
- **도구의 화면 이름은 하나다**: 도구는 어디서나 `name`으로 부르고 쉬운 설명(`plain_description`)은 caption이다(§7 gate-card 승인 카드·도구 목록·팔레트와 같은 규칙 — 한 도구를 두 이름으로 부르지 않는다)
- **노드 카드(실행 중, P4)**: 도구를 부르는 턴 동안 뱃지 "도구 부르는 중 {지금}/{최대}"(--text-caption, running 색). **마무리 호출 동안은 "답을 정리하는 중"**(숫자 없음 — 그 호출은 도구를 부르지 않으므로 턴 수로 세지 않는다: `n+1/n` 같은 말은 만들지 않는다). 사람 확인으로 멈춘 뒤 이어지는 동안에도 뱃지는 그대로 보인다(멈춤은 끝이 아니다). 최대 턴 수는 문서에 적힌 값, 안 적혔으면 **계약의 기본값**(registry `max_turns.default`)을 읽는다 — 화면이 숫자를 따로 적어 두지 않는다. `node.completed.closed_by`가 `turn_limit`·`tool_budget`이면 완료 뒤 캡션 "여기까지 알아본 것으로 답했어요"(warn 3층 — 실패가 아니다). `answer`면 캡션 없음
...
- review 상태(제안문이 핵심 — 왜 이렇게 바꾸자는지가 후보 검사보다 먼저 읽혀야 한다):
  ...
  - **모양 칩**(AGENT_PATTERNS P6, 제안문 묶음 안 대상 노드 칩 옆): 제안이 카탈로그의 모양을 가리키면 칩 하나로 그 모양의 짧은 이름을 말한다(--text-caption, surface-raised — 어그러진 자리가 아니므로 danger 계열 금지). 칩은 표시이지 손잡이가 아니다(누를 것이 없다). 모양을 가리키지 않는 제안에는 칩이 없다
