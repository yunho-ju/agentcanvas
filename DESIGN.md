...
- **화면 안이 먼저다**(UXQ2 백로그): 선택된 노드 오른쪽 자리가 지금 뷰포트에 온전히 들어오지 않으면 그 자리 대신 **뷰포트 안의 첫 빈 자리**(선택 없음 규칙과 같은 탐색)에 놓는다 — 연달아 놓을 때 화면이 한쪽으로 걸어가 먼저 놓은 카드를 화면 밖으로 밀지 않는다. 그래도 빈 자리가 없을 때만 위 이동 규칙이 든다. 사람이 세 장을 연달아 놓으면 세 장이 다 보인다(판정 기준)
