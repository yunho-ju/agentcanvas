...
- **지난 대화 두 뷰**(CHAT-4b): 이 패널 안에서 '지금 대화' ↔ '지난 대화'(목록)를 오간다 — 모드 항목도 새 패널도 늘리지 않는다(대화 목록은 대화의 일부다). 헤더 오른쪽에 뷰를 바꾸는 ghost 손잡이 1개('지난 대화' / '지금 대화'), 판 고정 표시는 '지금 대화' 뷰에서만 선다(목록은 어느 한 판의 이야기가 아니다). 대화 모드로 **처음 들어올 때 아직 오간 말이 없으면 목록 뷰로 연다** — 빈 화면 대신 이어 갈 대화를 먼저 보여 준다
