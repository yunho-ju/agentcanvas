...
- '최대 몇 턴'(`max_turns`)은 계약 기본값 1이 화면에도 1로 보인다(빈칸 아님 — 지금 엔진은 한 번에 답한다). 캡션(--text-caption, 상시 보임): "도구를 부르며 답을 다듬는 횟수예요. 1이면 한 번에 답해요 — 턴마다 모델 호출 비용이 들어요"(비용은 hover 뒤에 숨기지 않는다, §9)
...
- 엔진이 도구를 부르며 반복하기 전(P3)까지 이 두 칸은 **저장되는 값**이지 실행에 쓰이는 값이 아니다 — 그동안은 '최대 몇 턴' 캡션 끝에 '(이 서버는 아직 한 번에 답해요)'를 **무조건** 덧붙인다. 이 문장은 P3가 엔진에 들어갈 때 같은 커밋에서 빠진다(조건부가 아니라 제거다 — 서버와 화면은 한 배포다)
