...
- **현재 항목은 두 손 중 나중 것이 정한다**: 줄을 누르면 **그 줄**이 현재 항목이고 그 줄의 상세가 펼쳐진다 — 같은 시각(timestamp)의 사건이 여럿이어도 누른 줄이다(시각으로 되찾으면 마지막 사건이 열려 사람이 누른 것과 다른 것이 펼쳐진다 — 결함). 재생·되감기·스크럽이 움직이면 그때부터는 다시 시각이 현재 항목을 정한다. 누른 줄은 재생 위치도 그 사건의 시각으로 옮긴다(둘이 어긋나지 않는다)
