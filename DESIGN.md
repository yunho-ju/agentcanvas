...
- 메뉴: 저장 / **열기**(서버에 저장해 둔 문서 목록) / 파일 열기(내 컴퓨터의 파일) / 내보내기 / 정리하기. 문서가 없으면(빈 캔버스) 저장은 메뉴·단축키 **양쪽 다** 막고 이유를 말한다(title). 서버가 켜졌는지는 미리 알 수 없으므로 그것만은 시도 후 토스트로 알린다
...
- 빠진 말 토막(실패한 케이스를 펼쳤을 때만 — EVAL-1): 라벨 '답에 없던 말'(--text-label) + 기대 문구 중 **바로 위 결과 토막이 보여 주는 그 회차의 답**에 없던 것만 danger-ink 칩 목록. 답 A를 보여 주며 답 B의 빠진 말을 붙이지 않는다. ✕만 보여 주고 침묵하지 않는다 — 실패의 까닭이 위의 지시문 카드와 같은 화면에서 읽힌다. 판정 규칙은 서버 evaluator와 같다(NFC·대소문자 무시·연속 공백 1칸). 규칙대로 보면 빠진 말이 하나도 없는데 실패했다면 그것도 말하되 **지금 화면에 있는 손잡이**를 가리킨다: '어느 말이 빠졌는지 찾지 못했어요 — 자세히 보기를 켜면 회차마다 무엇이 나왔는지 볼 수 있어요'(없는 곳을 가리키지 않는다)
...
- 기술 정보: evaluator 이름·버전과 run id는 전문가 보기에서만 mono caption으로 보조 노출한다. 서버 원문 오류·JSON 봉투·없는 점수는 만들지 않는다
