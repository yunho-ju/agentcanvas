...
| 단계 | 내용 | 커밋 |
|---|---|---|
| P1 | '쓸 도구'·'최대 몇 턴'이 거짓 약속을 멈춤(체크 목록, 잠긴 이유를 보이는 줄로) | e03e2b1 |
| P2 | 모델 호출 계약에 도구·이전 턴·도구 호출 추가, OpenAI·Anthropic 어댑터, 카탈로그 `tool_calling`·`tools_need_thinking_off` | 538b4d4 |
| P3 | 엔진 루프(턴·한도·마무리 호출·승인 멈춤/재개·펜싱), `RunEvent.turn`, 말한 것 규칙 양쪽 | 2a10e7e |
| P4 | 실행 보기 시도 묶음·노드 뱃지 | 12a6063 |
| P5 | 패턴 카탈로그 registry, `GET /patterns`, 감지·템플릿 채우기 | e3781d6, 80cd595 |
| P6c | 고치기 패널의 모양 칩 | 1b62b65, 1d5022e |
| P6a | Architect 되묻기, 가짜 실행이 밸브를 지나감, 두 카드 한 게이트 | 78d984a |
| P6b | 팔레트 '이 모양으로 놓기', 앵커 규칙 미러 | ec0684f |

실키로 확인한 것: 도구 호출 2콜 왕복(P2), 엔진이 2턴에 답함(P3), 실행 보기 시도 묶음(P4), 고치기 제안이 모양을 고름(P6c), Architect 되묻기 두 갈래(P6a). 팔레트는 모델 호출 없이 확인(P6b).

## 11. 남은 것 (백로그)
- 빈 곳 오른쪽 클릭 메뉴의 '이 모양으로 놓기' 하위 목록(같은 `putPattern` 문 호출).
- `{new:…}` 앵커를 선의 끝으로 쓰는 템플릿은 엔진과 화면의 답이 다르다(엔진 `missing_node`, 화면은 놓고 첫 포트로 이음) — 지금 카탈로그에는 그런 템플릿이 없다. `examples/pattern-anchors/README.md`의 '맞추지 않는 것' 절 참조.
- 좁은 창에서 모양을 놓으면 '화면 안이 먼저다'가 줄 순서를 이겨 선이 길어질 수 있다. 뒤집으려면 DESIGN이 "모양 놓기는 줄 순서가 먼저"라고 먼저 말해야 한다.
- 병렬 실행·합류, 다른 에이전트 부르기(서브에이전트·A2A), 반복(for-each), LangGraph 어댑터, MCP 서버 노출, 토큰·시간 한도 집행 — §9 그대로.
